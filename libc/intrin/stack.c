...
/**
 * Gets maximum number of stacks cosmo should cache.
...
/**
 * Sets maximum number of stacks cosmo should cache.
 *
 ...
 *
 * By default, sixteen stacks may be cached at any given moment.
 *
 ...
 * `guardsize` parameters are an exact match. Otherwise they'll likely
 * be freed eventually, in a least-recently used fashion, based upon the
 * configurable cosmo_stack_setmaxstacks() setting.
 *
...
errno_t cosmo_stack_free(void *stackaddr, size_t stacksize, size_t guardsize) {
  stacksize = (stacksize + __gransize - 1) & -__gransize;
  guardsize = (guardsize + __pagesize - 1) & -__pagesize;
  unassert(stackaddr && !((uintptr_t)stackaddr & (__pagesize - 1)));
  unassert(stacksize);
